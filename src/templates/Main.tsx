...
import { ContextStates } from '@/contexts';
...
const Main = (props: IMainProps) => {
  const { states, setStates } = useContext<any>(ContextStates as any);

  const handleThemeToggle = () => {
    setStates((prevStates: IStateProps) => ({
      ...prevStates,
      isDarkMode: !prevStates.isDarkMode,
    }));
  };
  ...
  return (
    <main className="grid h-screen w-screen grid-rows-[3fr_2fr] md:grid-cols-[3fr_2fr]">
      ...
      <section
        className={`flex h-full w-full flex-col md:h-screen ${
          states.isDarkMode ? 'bg-teal-100' : ''
        }`}
      >
        <div className="flex grow flex-col justify-center text-right">
          ...
          <Banner handleThemeToggle={handleThemeToggle} />
          <div className="flex justify-end">
            <Link href="/services">
